...
export function PartsManagementModal({
  ...
  const handleAddNew = async () => {
    // Auto-generate ECS Serial Number
    let autoGeneratedSerial = "";
    if (job?.shopName) {
      try {
        const shopCode = getShopCode(job.shopName);
        const dateCode = getTodayDateCode();
        
        // Call API to generate next sequential serial number
        const response = await apiRequest(`/api/serial/generate`, {
          method: "POST",
          body: JSON.stringify({ shopCode, date: dateCode }),
          headers: { "Content-Type": "application/json" },
        });
        
        if (response.ok) {
          const data = await response.json();
          autoGeneratedSerial = data.serialNumber;
        }
      } catch (error) {
        console.error("Failed to auto-generate serial number:", error);
        // Continue without auto-generated serial - user can enter manually
      }
    }
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Input {...field} data-testid="input-ecs-serial" placeholder="Auto-generated (editable)" />
                        </FormControl>
                        <p className="text-xs text-muted-foreground mt-1">
                          Format: XX.MMDDYYYY.ZZ (e.g., 01.11232025.03)
                        </p>
                        <FormMessage />
                      </FormItem>
                    )}
