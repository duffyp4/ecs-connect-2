...
- **Test data should always produce happy path**: Random Pass/Fail and partial submission statuses make test data frustrating — you have to manually fix each part before you can test submission. Fixed by forcing `passOrFail="Pass"` and `submissionStatus="Completed"` as a final override step in the generator. Developers who want to test failure paths can manually flip one part after filling.
- **Sign-off validation should be comprehensive**: The original sign-off warning only checked `submissionStatus !== "Completed"`. Expanded to also check `passOrFail` is set and `failedReason` is provided when `passOrFail === "Fail"`. Uses a structured issue list with per-part labels (e.g., "DPF: Pass or Fail not selected") instead of a generic count.
